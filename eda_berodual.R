...
#  7.1_IMS data_Berodual.skv
sales <- read.delim("movalis/7.1_IMS data_Berodual2.skv",header=TRUE,sep=";",dec=",",
                    colClasses=c("factor","factor","factor","factor","numeric","numeric",
                                 "factor","factor","factor","factor","factor","factor"
                                 ,"numeric","factor","factor","factor","factor","factor","factor"
                                 ,"factor","factor","character","factor","factor","factor"
...
sales$ATC.ATC_1 <- NULL # R RESPIRATORY SYSTEM
sales$ATC.ATC_2 <- NULL # R03 ANTI-ASTHMA & COPD PROD
sales$Corporation.Producer_Name <- NULL # missing
sales$OTC_Name.OTC_Name <- NULL # PRESC

# BERODUAL sales by month
table(sales$Brands.TradeName[sales$Brands.Brand=="BERODUAL"])
table(sales$Brands.Brand[sales$Brands.TradeName=="BERODUAL"])

sales_berodual <- aggregate(sales[sales$Brands.Brand=="BERODUAL",c("Units.month","Sales.LC.MTH")],
                           list(sales$Time.AsOfDate[sales$Brands.Brand=="BERODUAL"]),sum)
names(sales_berodual) <- c("Month","Units","Value")

par(mar = c(5, 4, 4, 4))
plot(sales_berodual$Month,sales_berodual$Units/1e5,type="l",col="blue",xlab="",
     ylab="Units (00,000s)",
     main="Berodual Sales Russia",ylim=c(0,max(sales_berodual$Units/1e5)))
par(new = TRUE)
plot(sales_berodual$Month, sales_berodual$Value/1e6, type="l", col="black", axes=FALSE, 
     xlab="", ylab="", ylim=c(0,max(sales_berodual$Value/1e6)))
axis(side=4, at = pretty(c(0,200)))
mtext("Value (000,000s)", side=4, line=3)
legend("bottomright",c("Units","Value"),col=c("blue","black"),lty=1)

# decomposition
plot(decompose(ts(sales_berodual$Units/1e6,start=c(2009,1),frequency=12)))

# truncate
idx<-format(sales_berodual$Month,format="%Y")%in%c("2009","2010")
plot(sales_berodual$Month[idx],sales_berodual$Units[idx]/1e5,type="l",col="blue",xlab="",
     ylab="Units (00,000s)",
     main="Berodual Sales Russia",ylim=c(0,max(sales_berodual$Units[idx]/1e5)))
idx<-as.numeric(format(sales_berodual$Month,format="%Y"))>=2010
sales_berodual<-sales_berodual[idx,]

# ARIMA forecasting
n <- length(sales_berodual$Units)
print(n)
ntrain <- 36
ntest <- 12
...
for(i in 1:ntrials) {
  train <- sales_berodual$Units[i:(i+ntrain-1)]
  test <- sales_berodual$Units[(i+ntrain):(i+ntrain+ntest-1)]
